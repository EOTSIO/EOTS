...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(eotio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(eotio.token), eotio_token_wast );
   c.set_abi( N(eotio.token), eotio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(eotio.token), N(create), N(eotio.token), mutable_variant_object()
              ("issuer",       "eotio" )
   ...
   cr = c.push_action( N(eotio.token), N(issue), N(eotio), mutable_variant_object()
              ("to",       "dan" )
