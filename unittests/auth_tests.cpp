...
#include <eotio/chain/resource_limits_private.hpp>
...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for eotio reqauth action with alice's spending key
   chain.link_authority("alice", "eotio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "eotio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with eotio reqauth
   chain.unlink_authority("alice", "eotio", "reqauth");
   ...
   // Link authority for any eotio action with alice's scud key
   chain.link_authority("alice", "eotio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "eotio", "first",  "reqauth");
...
try {
   ...
   // Creating account with eotio. prefix with privileged account
   chain.create_account("eotio.test1");

   // Creating account with eotio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("eotio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'eotio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "eotio", "eotio.any", "reqauth" );
   chain.link_authority( "bob", "eotio", "eotio.any", "reqauth" );

   /// this should succeed because eotio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = eotio::chain::resource_limits::resource_usage_object;
   using by_owner = eotio::chain::resource_limits::by_owner;
