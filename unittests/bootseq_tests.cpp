...
public:
   ...
   fc::variant get_global_state() {
      vector<char> data = get_row_by_account( N(eotio), N(eotio), N(global), N(global) );
      if (data.empty()) std::cout << "\nData is empty\n" << std::endl;
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "eotio_global_state", data );

   }

    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(eotio), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(eotio), N(delegatebw), from, mvo()
                    ("from", "eotio" )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(eotio), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(eotio), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(eotio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create eotio.msig and eotio.token
        create_accounts({N(eotio.msig), N(eotio.token)});

        // Set code for the following accounts:
        //  - eotio (code: eotio.bios) (already set by tester constructor)
        //  - eotio.msig (code: eotio.msig)
        //  - eotio.token (code: eotio.token)
        set_code_abi(N(eotio.msig), eotio_msig_wast, eotio_msig_abi);//, &eotio_active_pk);
        set_code_abi(N(eotio.token), eotio_token_wast, eotio_token_abi); //, &eotio_active_pk);

        // Set privileged for eotio.msig and eotio.token
        set_privileged(N(eotio.msig));
        set_privileged(N(eotio.token));

        // Verify eotio.msig and eotio.token is privileged
        const auto& eotio_msig_acc = get<account_object, by_name>(N(eotio.msig));
        BOOST_TEST(eotio_msig_acc.privileged == true);
        const auto& eotio_token_acc = get<account_object, by_name>(N(eotio.token));
        BOOST_TEST(eotio_token_acc.privileged == true);


        // Create SYS tokens in eotio.token, set its manager as eotio
        ...
        create_currency(N(eotio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to eotio.system
        issue(N(eotio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(eotio) );
        }

        // Set eotio.system to eotio
        set_code_abi(N(eotio), eotio_system_wast, eotio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(eotio), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(eotio), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(eotio), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "eotio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), eotio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), eotio_assert_message_exception);
