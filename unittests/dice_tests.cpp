...
FC_REFLECT(account_t, (owner)(eot_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(eotio.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(eotio.token), N(create), N(eotio.token), mvo()
     ("issuer", "eotio.token")
     ("maximum_supply", core_from_string("1000000000.0000"))
   );

   push_action(N(eotio.token), N(issue), N(eotio.token), mvo()
     ("to", "eotio")
     ("quantity", core_from_string("1000000000.0000"))
     ("memo", "")
   );

   transfer( N(eotio), N(alice), core_from_string("10000.0000"), "", N(eotio.token) );
   transfer( N(eotio), N(bob),   core_from_string("10000.0000"), "", N(eotio.token) );
   transfer( N(eotio), N(carol), core_from_string("10000.0000"), "", N(eotio.token) );
   ...
   BOOST_REQUIRE_EQUAL( 
      get_currency_balance(N(eotio.token), symbol(CORE_SYMBOL), N(alice)),
