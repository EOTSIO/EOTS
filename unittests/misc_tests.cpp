...
#include <eotio/chain/authority.hpp>
#include <eotio/chain/types.hpp>
#include <eotio/chain/asset.hpp>
...
BOOST_AUTO_TEST_CASE(transaction_test) { try {
   ...
   ((const signed_transaction &)trx).sign( test.get_private_key( N(eotio), "active" ), chain_id_type());
   BOOST_CHECK_EQUAL(0, trx.signatures.size());
   trx.sign( test.get_private_key( N(eotio), "active" ), chain_id_type()  );
