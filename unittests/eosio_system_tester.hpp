...
namespace eotio_system {

class eotio_system_tester : public TESTER {
public:

   eotio_system_tester()
   : eotio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   eotio_system_tester(Lambda setup) {
      setup(*this);
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(eotio.token) );
         abi_def abi;
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& eotin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",eotin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      ...
      vector<char> data = get_row_by_account( N(eotio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = eotio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(eotio.token), symbol_code, N(stat), symbol_code );
...
inline uint64_t M( const string& eot_str ) {
   return core_from_string( eot_str ).get_amount();
}
