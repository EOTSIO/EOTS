...
Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping eotd instances, but with whom you may wish to collaborate for setting up a decentralized testnet.
...
Note that the Launcher will not push instances of eotd to the remote hosts, you must prepare the various test network hosts separately.
...
To address the complexity implied by distributing multiple eotd nodes across a LAN or a wider network, the launcher application was created. 

Based on a handful of command line arguments the Launcher is able to compose per-node configuration files, distribute these files securely amongst the peer hosts, then start up the multiple instances of eotd.

eotd instances started this way have their output logged in individual text files. Finally the launcher application is also able to shut down some or all of the test network. 

## Running the Launcher application

The launcher program is used to configure and deploy producing and non-producing eotd nodes that talk to each other using configured routes. The configuration for each node is stored in separate directories, permitting multiple nodes to be active on the same host, assuming the machine has sufficient memory and disk space for multiple eotd instances. The launcher makes use of multiple configuration sources in order to deploy a testnet. A handful of command line arguments can be used to set up simple local networks. 
...
launcher command line arguments:
  ...
  -d [ --delay ] arg (=0)               number of seconds to wait before starting the next node. Used to simulate a person keying in a series of individual eotd startup command lines.
  ...
  -o [ --output ] arg                   save a copy of the generated topology 
                                        in this file
  --skip-signature                      eotD does not require transaction 
                                        signatures.
...
In this mode, the launcher does not activate any eotd instances, it produces a file of the given filename. This file is a JSON formatted template that provides an easy means of 
...
```
  "nodes": [[
      "testnet_0",{
        ...
        "filesize": 8192,
        "keys": [{
            "public_key": "eot6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
| nodes         | a collection of descriptors defining the eotd instances used to assemble this testnet. The names used as keys in this collection are also aliases used within as placeholders for peer nodes.
...
| eot_root_dir       | specifies the directory into which all eotd artifacts are based. This is required for any hosts that are not the local host.
...
| :------------ | :----------
| config.ini    | The eotd configuration file.
| eotd.pid      | The process ID of the running eotd instance.
| blockchain/*  | The blockchain backing store
| blocks/*      | The blockchain log store
| stderr.txt    | The cerr output from eotd.
| stdout.txt    | The cout output from eotd.
