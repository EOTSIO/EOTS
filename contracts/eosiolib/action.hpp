...
namespace eotio {
    ...
    *    int  c; //4
    *
    *    eotLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      eotLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      eotLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         eotio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         eotio_assert( name == T::get_name(), "Invalid name" );
         eotio_assert( account == T::get_account(), "Invalid account" );
...
::eotio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::eotio::string_to_name(#NAME) )
...
#define ACTION( CODE, NAME ) struct NAME : ::eotio::action_meta<CODE, ::eotio::string_to_name(#NAME) >
