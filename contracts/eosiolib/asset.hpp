...
namespace eotio {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         eotio_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         eotio_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         eotio_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         eotio_assert( -max_amount <= amount, "subtraction underflow" );
         eotio_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }

      asset& operator+=( const asset& a ) {
         eotio_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         eotio_assert( -max_amount <= amount, "addition underflow" );
         eotio_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         eotio_assert( a == 0 || (amount * a) / a == amount, "multiplication overflow or underflow" );
         eotio_assert( -max_amount <= amount, "multiplication underflow" );
         eotio_assert( amount <= max_amount,  "multiplication overflow" );
         amount *= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         eotio_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      eotLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         eotio_assert( a.contract == b.contract, "type mismatch" );
      ...
      eotLIB_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
