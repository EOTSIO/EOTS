...
namespace eotio {
   ...
   void set_blockchain_parameters(const eotio::blockchain_parameters& params) {
      char buf[sizeof(eotio::blockchain_parameters)];
      eotio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(eotio::blockchain_parameters& params) {
      char buf[sizeof(eotio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      eotio_assert( size <= sizeof(buf), "buffer is too small" );
      eotio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   */
   ...
   private:
      ...
      public:
         ...
         {
            eotio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return eotio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = eotio::memory_heap.malloc(count*size);
...
{
   return eotio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return eotio::memory_heap.free(ptr);
}
