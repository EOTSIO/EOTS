   ...
   class currency {
      public:
         ...
         struct create {
           ...
           // array<char,32>         issuer_agreement_hash;
            ...
            eotLIB_SERIALIZE( create, (issuer)(maximum_supply)(issuer_can_freeze)(issuer_can_recall)(issuer_can_whitelist) )
         };
         ...
         struct transfer
         {
            ...
            eotLIB_SERIALIZE( transfer, (from)(to)(quantity)(memo) )
         };
         ...
         struct issue {
            ...
            eotLIB_SERIALIZE( issue, (to)(quantity)(memo) )
         };
         ...
         struct fee_schedule {
            ...
            eotLIB_SERIALIZE( fee_schedule, (fee_per_length) )
         };
         ...
         struct account {
            ...
            eotLIB_SERIALIZE( account, (balance)(frozen)(whitelist) )
         };
         ...
         struct currency_stats {
            ...
            eotLIB_SERIALIZE( currency_stats, (supply)(max_supply)(issuer)(can_freeze)(can_recall)(can_whitelist)(is_frozen)(enforce_whitelist) )
         };
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                eotio_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             eotio_assert( i.quantity.is_valid(), "invalid quantity" );
             eotio_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             eotio_assert( t.quantity.is_valid(), "invalid quantity" );
             eotio_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             if( has_auth( owner ) ) {
                eotio_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                eotio_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                eotio_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                eotio_assert( st.can_recall, "issuer may not recall token" );
             } else {
                eotio_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                eotio_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                eotio_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
