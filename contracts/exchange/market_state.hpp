...
namespace eotio {
   ...
   struct margin_position {
      ...
      eotLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef eotio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), eotio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      eotLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef eotio::multi_index<N(loans), loan_position> loans;
