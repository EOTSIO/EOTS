...
namespace eotiosystem {
   ...
   using eotio::block_timestamp;

   struct eotio_parameters : eotio::blockchain_parameters {
      uint64_t          max_ram_size = 64ll*1024 * 1024 * 1024;

      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE_DERIVED( eotio_parameters, eotio::blockchain_parameters, (max_ram_size) )
   };
   ...
   typedef eotio::multi_index< N(namebids), name_bid,
   ...
   struct eotio_global_state : eotio_parameters {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE_DERIVED( eotio_global_state, eotio_parameters, (total_ram_bytes_reserved)(total_ram_stake)
   ...
   struct producer_info {
      ...
      eotio::public_key     producer_key; /// a packed public key object
      std::string           url;
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE( producer_info, (owner)(total_votes)(producer_key)(url)
   ...
   struct voter_info {
      ...
      eotio::asset                unstaking; /// the total unstaking (pending 3 day delay)

      uint64_t primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE( voter_info, (owner)(proxy)(producers)(staked)(last_vote_weight)(proxied_vote_weight)(is_proxy)(deferred_trx_id)(last_unstake_time)(unstaking) )
   };

   typedef eotio::multi_index< N(voters), voter_info>  voters_table;


   typedef eotio::multi_index< N(producers), producer_info,
                               indexed_by<N(prototalvote), const_mem_fun<producer_info, double, &producer_info::by_votes>  >
                               >  producers_table;

   typedef eotio::singleton<N(global), eotio_global_state> global_state_singleton;
   ...
   class system_contract : public native {
      ...
      private:
         ...
         static eotio_global_state get_default_parameters();
