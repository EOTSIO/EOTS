...
namespace eotiosystem {
   using eotio::asset;
   using eotio::indexed_by;
   using eotio::const_mem_fun;
   using eotio::bytes;
   using eotio::print;
   using eotio::permission_level;
   using std::map;
   using std::pair;
   ...
   struct user_resources {
      ...
      eotLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      eotio::asset  net_amount;
      eotio::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      eotLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef eotio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef eotio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef eotio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto eotout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, eotout );
   }
   ...
   {
      require_auth( payer );
      eotio_assert( quant.amount > 0, "must purchase a positive amount" );

      if( payer != N(eotio) ) {
         INLINE_ACTION_SENDER(eotio::token, transfer)( N(eotio.token), {payer,N(active)},
                                                       { payer, N(eotio), quant, std::string("buy ram") } );
      }
      ...
      eotio_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      eotio_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      eotio_assert( res_itr != userres.end(), "no resource row" );
      eotio_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      eotio_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      if( N(eotio) != account ) {
         INLINE_ACTION_SENDER(eotio::token, transfer)( N(eotio.token), {N(eotio),N(active)},
                                                       { N(eotio), account, asset(tokens_out), std::string("sell ram") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      eotio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      eotio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         eotio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         eotio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         eotio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         eotio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(eotio) != source_stake_from ) { //for eotio both transfer and refund make no sense
         ...
         if ( req != refunds_tbl.end() ) { //need to update refund
            ...
            eotio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
            eotio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(eotio::token, transfer)( N(eotio.token), {from,N(active)},
               { source_stake_from, N(eotio), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         eotio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      eotio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      eotio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      eotio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
   ...
   {
      eotio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      eotio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      eotio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      eotio_assert( req != refunds_tbl.end(), "refund request not found" );
      eotio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(eotio::token, transfer)( N(eotio.token), {N(eotio),N(active)},
                                                    { N(eotio), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
