...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // If eotIO_ABI macro was found, check if the current declaration
  ...
  // The current Decl was the type referenced in eotIO_ABI macro
  if( type_has_actions ) return;

  // The current Decl was not the type referenced in eotIO_ABI macro
  ...
  // If eotIO_ABI macro was found, we will only check if the current Decl
