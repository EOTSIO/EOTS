...
#include <eotio/chain/block_context.hpp>
#include <eotio/chain/transaction_context.hpp>

#include <eotio/chain/block_log.hpp>
...
#include <eotio/chain/transaction_object.hpp>
#include <eotio/chain/unconfirmed_block_object.hpp>
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( eotio, eotio, newaccount );
   SET_APP_HANDLER( eotio, eotio, setcode );
   SET_APP_HANDLER( eotio, eotio, setabi );
   SET_APP_HANDLER( eotio, eotio, updateauth );
   SET_APP_HANDLER( eotio, eotio, deleteauth );
   SET_APP_HANDLER( eotio, eotio, linkauth );
   SET_APP_HANDLER( eotio, eotio, unlinkauth );
/*
   SET_APP_HANDLER( eotio, eotio, postrecovery );
   SET_APP_HANDLER( eotio, eotio, passrecovery );
   SET_APP_HANDLER( eotio, eotio, vetorecovery );
*/

   SET_APP_HANDLER( eotio, eotio, canceldelay );
...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
   ...
   void initialize_fork_db() {
      ...
      producer_schedule_type initial_schedule{ 0, {{N(eotio), conf.genesis.initial_key}} };
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(eotio_contract_abi(abi_def()));
         }
      });
  ...
  //    ilog((fc::json::to_pretty_string(*pending->_pending_block_state->block)));
   ...
   void finalize_block()
   {
      ...
      */
      ...
      uint64_t CPU_TARGET = eot_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {eot_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      eot_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   eot_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
void controller::validate_referenced_accounts( const transaction& trx )const {
   for( const auto& a : trx.context_free_actions ) {
      ...
      eot_ASSERT( code != nullptr, transaction_exception,
                  "action's code account '${account}' does not exist", ("account", a.account) );
      eot_ASSERT( a.authorization.size() == 0, transaction_exception,
   ...
   for( const auto& a : trx.actions ) {
      ...
      for( const auto& auth : a.authorization ) {
         ...
         eot_ASSERT( actor  != nullptr, transaction_exception,
                     "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
         eot_ASSERT( my->authorization.find_permission(auth) != nullptr, transaction_exception,
                     "action's authorizations include a non-existent permission: {permission}",
                     ("permission", auth) );
      }
   }
   eot_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
}

void controller::validate_expiration( const transaction& trx )const { try {
   const auto& chain_configuration = get_global_properties().configuration;

   eot_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   eot_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
