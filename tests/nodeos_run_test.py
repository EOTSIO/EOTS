...
###############################################################
# nodeot_run_test
# --dump-error-details <Upon error print etc/eotio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
parser.add_argument("-h", "--host", type=str, help="%s host name" % (testUtils.Utils.eotServerName),
                    default=LOCAL_HOST)
parser.add_argument("-p", "--port", type=int, help="%s host port" % testUtils.Utils.eotServerName,
...
parser.add_argument("--dump-error-details",
                    help="Upon error print etc/eotio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                    action='store_true')
...
try:
    ...
    # create accounts via eotio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.eotioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.eotioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.eotioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.eotioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.eotioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.eotioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    actualAmount=node.getAccounteotBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccounteotBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/eotio.token"
    wastFile="contracts/eotio.token/eotio.token.wast"
    abiFile="contracts/eotio.token/eotio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.geteotAccountFromDb(currencyAccount.name)
    ...
    account=node.geteotAccount(defproduceraAccount.name)
    if account is None:
