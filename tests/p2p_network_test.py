...
# eotio should have the same key as defproducera
eotio = copy.copy(defproduceraAccount)
eotio.name = "eotio"

Print("Info of each node:")
for i in range(len(hosts)):
    node = cluster.getNode(0)
    cmd="%s %s get info" % (testUtils.Utils.eotClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wastFile="contracts/eotio.system/eotio.system.wast"
abiFile="contracts/eotio.system/eotio.system.abi"
Print("\nPush system contract %s %s" % (wastFile, abiFile))
trans=node0.publishContract(eotio.name, wastFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish eotio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, eotio)
