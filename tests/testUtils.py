...
class Utils:
    ...
    eotClientPath="programs/cleot/cleot"

    eotWalletName="keotd"
    eotWalletPath="programs/keotd/"+ eotWalletName

    eotServerName="nodeot"
    eotServerPath="programs/nodeot/"+ eotServerName

    eotLauncherPath="programs/eotio-launcher/eotio-launcher"
    MongoPath="mongo"
    ...
    # mongoSyncTime: nodeot mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="eottest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.geteotAccount(account.name)
            try:
                assert(accountInfo)
    ...
    def getBlock(self, blockNum, retry=True, silentErrors=False):
        ...
        if not self.enableMongo:
            cmd="%s %s get block %s" % (Utils.eotClientPath, self.endpointArgs, blockNum)
    ...
    def getTransaction(self, transId, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get transaction %s" % (Utils.eotClientPath, self.endpointArgs, transId)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                trans=Node.runCmdReturnJson(cmd)
                return trans
    ...
    def geteotAccount(self, name):
        assert(isinstance(name, str))
        cmd="%s %s get account -j %s" % (Utils.eotClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def geteotAccountFromDb(self, name):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table):
        cmd="%s %s get table %s %s %s" % (Utils.eotClientPath, self.endpointArgs, contract, scope, table)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL):
        ...
        cmd="%s %s get currency balance %s %s %s" % (Utils.eotClientPath, self.endpointArgs, contract, account, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getCurrencyStats(self, contract, symbol=CORE_SYMBOL):
        ...
        assert(symbol)
        assert(isinstance(symbol, str))
        cmd="%s %s get currency stats %s %s" % (Utils.eotClientPath, self.endpointArgs, contract, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def verifyAccount(self, account):
        if not self.enableMongo:
            ret=self.geteotAccount(account.name)
            if ret is not None:
            ...
            for _ in range(2):
                ret=self.geteotAccountFromDb(account.name)
                if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.eotClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.geteotBalances([source] + accounts)
    ...
    def geteotBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccounteotBalance(account.name)
    ...
    def getAccountsByKey(self, key):
        cmd="%s %s get accounts %s" % (Utils.eotClientPath, self.endpointArgs, key)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    # Get actions mapped to an account (cleot get actions)
    def getActions(self, account, pos=-1, offset=-1):
        assert(isinstance(account, Account))
        assert(isinstance(pos, int))
        assert(isinstance(offset, int))

        cmd="%s %s get actions -j %s %d %d" % (Utils.eotClientPath, self.endpointArgs, account.name, pos, offset)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getServants(self, name):
        cmd="%s %s get servants %s" % (Utils.eotClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccounteotBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from cleot get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        if not self.enableMongo:
            amount=self.getTableAccountBalance("eotio.token", scope)
            if Utils.Debug: Utils.Print("getNodeAccounteotBalance %s %s" % (scope, amount))
            ...
            account=self.geteotAccountFromDb(scope)
            if account is not None:
                balance=account["eot_balance"]
                return balance

        return None

    def getAccounteotBalance(self, scope):
        """Returns SYS currency0000 account balance from cleot get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccounteotBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.eotClientPath, self.endpointArgs, account)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def publishContract(self, account, contractDir, wastFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.eotClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.eotClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False):
        cmd="%s %s set action permission -j %s %s %s %s" % (
            Utils.eotClientPath, self.endpointArgs, account, code, pType, requirement)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Node.runCmdReturnJson(cmd)
    ...
    def getInfo(self, silentErrors=False):
        cmd="%s %s get info" % (Utils.eotClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by cleot get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
...
class WalletMgr(object):
    __walletLogFile="test_keotd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(keotd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodeotPort=8888, nodeotHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.nodeotPort=nodeotPort
        self.nodeotHost=nodeotHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodeotHost, self.nodeotPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch keotd")
            return False

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d" % (
            Utils.eotWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        with open(WalletMgr.__walletLogFile, 'w') as sout, open(WalletMgr.__walletLogFile, 'w') as serr:
            popen=subprocess.Popen(cmd.split(), stdout=sout, stderr=serr)
            self.__walletPid=popen.pid

        # Give keotd time to warm up
        time.sleep(1)
        return True
    ...
    def create(self, name):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s" % (Utils.eotClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def importKey(self, account, wallet):
        ...
        cmd="%s %s wallet import --name %s %s" % (
            Utils.eotClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s %s" % (
                Utils.eotClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                subprocess.check_output(cmd.split(), stderr=subprocess.STDOUT).decode("utf-8")
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.eotClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.eotClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.eotClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.eotClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=subprocess.check_output(cmd.split()).decode("utf-8")
    ...
    def getKeys(self):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet keys" % (Utils.eotClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=subprocess.check_output(cmd.split()).decode("utf-8")
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    @staticmethod
    def killall():
        cmd="pkill -9 %s" % (Utils.eotWalletName)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
...
class Cluster(object):
    ...
    # pylint: disable=too-many-arguments
    # walletd [True|False] Is keotd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="eottest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        ...
        self.eotioAccount=Account("eotio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontKill=False):
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.eotServerName))
            return True
        ...
        cmd="%s -p %s -n %s -s %s -d %s -f" % (
            Utils.eotLauncherPath, pnodes, totalNodes, topo, delay)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodeotArgs="--max-transaction-time 5000 --filter-on *"
        if not self.walletd:
            nodeotArgs += " --plugin eotio::wallet_api_plugin"
        if self.enableMongo:
            nodeotArgs += " --plugin eotio::mongo_db_plugin --resync --mongodb-uri %s" % self.mongoUri

        if nodeotArgs:
            cmdArr.append("--nodeot")
            cmdArr.append(nodeotArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.eotServerName, totalNodes, len(nodes)))
            return False
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key" % (Utils.eotClientPath)
                if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nexteotIdx: %d, n: %d" % (nexteotIdx, n))
                nexteotIdx=(nexteotIdx + 1)%count
                if not self.nodes[nexteotIdx].killed:
                    #Utils.Print("nexteotIdx: %d" % (nexteotIdx))
            ...
            #Utils.Print("nexteotIdx: %d, count: %d" % (nexteotIdx, count))
            node=self.nodes[nexteotIdx]
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].geteotBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.eotioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        node="node_bios"
        configFile="etc/eotio/%s/config.ini" % (node)
        if Utils.Debug: Utils.Print("Parsing config file %s" % configFile)
        producerKeys=Cluster.parseProducerKeys(configFile, node)
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse eotio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            eotioName="eotio"
            eotioKeys=producerKeys[eotioName]
            eotioAccount=Account(eotioName)
            eotioAccount.ownerPrivateKey=eotioKeys["private"]
            eotioAccount.ownerPublicKey=eotioKeys["public"]
            eotioAccount.activePrivateKey=eotioKeys["private"]
            eotioAccount.activePublicKey=eotioKeys["public"]

            if not walletMgr.importKey(eotioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (eotioName))
                return False
            ...
            trans=biosNode.publishContract(eotioAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, eotioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        myTrans=biosNode.pushMessage("eotio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("eotio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but eotio).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "eotio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            eotioTokenAccount=copy.deepcopy(eotioAccount)
            eotioTokenAccount.name="eotio.token"
            trans=biosNode.createAccount(eotioTokenAccount, eotioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (eotioTokenAccount.name))
                return False
            ...
            contract="eotio.token"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(eotioTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False
            
            # Create currency0000, followed by issue currency0000
            contract=eotioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (eotioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to eotio contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (eotioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to eotio contract.")
                return False
            ...
            Utils.Print("Verify eotio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccounteotBalanceStr(eotioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return False

            contract="eotio.system"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (eotioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (eotioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (eotioTokenAccount.name, name))
                    return False
    ...
    # Populates list of eotInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.eotServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.eotServerName, pattern))
                break
    ...
    def killSomeeotInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        ...
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.eotServerName, killSignal))
    ...
    def dumpErrorDetails(self):
        fileName="etc/eotio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/eotio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/eotio/node_*"):
            shutil.rmtree(f)
