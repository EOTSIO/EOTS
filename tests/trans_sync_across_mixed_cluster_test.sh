...
killAll()
{
  programs/eotio-launcher/eotio-launcher -k 15
}

cleanup()
{
    rm -rf etc/eotio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cleot/cleot get info)"
  verifyErrorCode "cleot get info"
...
# stand up nodeot cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodeot \"--plugin eotio::wallet_api_plugin\" $launcherOpts
programs/eotio-launcher/eotio-launcher --nodeot "--plugin eotio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cleot/cleot --port $port2 get block 1)"
    verifyErrorCode "cleot get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cleot/cleot create key)"
verifyErrorCode "cleot create key"
...
PASSWORD_INITA="$(programs/cleot/cleot wallet create --name inita)"
verifyErrorCode "cleot wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cleot/cleot wallet import --name inita $INITA_PRV_KEY
verifyErrorCode "cleot wallet import"
programs/cleot/cleot wallet import --name inita $PRV_KEY1
verifyErrorCode "cleot wallet import"
programs/cleot/cleot wallet import --name inita $PRV_KEY2
verifyErrorCode "cleot wallet import"
programs/cleot/cleot wallet import --name inita $PRV_KEY3
verifyErrorCode "cleot wallet import"
...
ACCOUNT_INFO="$(programs/cleot/cleot create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cleot create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cleot/cleot get account testera)"
verifyErrorCode "cleot get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cleot/cleot transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cleot transfer"
   ...
   TRANS_INFO="$(programs/cleot/cleot --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cleot get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
