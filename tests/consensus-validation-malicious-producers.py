...
private-key = ["eot6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"]
producer-name = initu
plugin = eotio::producer_plugin
plugin = eotio::chain_api_plugin
plugin = eotio::account_history_plugin
plugin = eotio::account_history_api_plugin"""
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/eotio/node_%02d" % (count))
...
killeotInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up EOTS cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up EOTS walletd.")
            return False
